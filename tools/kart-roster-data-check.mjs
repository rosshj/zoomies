...
assert.equal(KART_LIVERIES.length,8);assert.equal(KART_PRESETS.length,34);assert.equal(KART_STYLES.length,17);
...
for(const [i,k] of KART_PRESETS.entries()){
 ...
 assert.ok(KART_LIVERIES[k.livery]);
 if(i>=10){const p=defaultProfile();p.treats=1000;assert.ok(buyUnlock(p,`kart.${i}`));assert.ok(p.unlocked.includes(`kart.${i}`));}
}
for(let style=5;style<17;style++){
 const rows=KART_PRESETS.filter(k=>k.style===style);assert.equal(rows.length,2);assert.notDeepEqual([rows[0].color,rows[0].livery],[rows[1].color,rows[1].livery]);
}
...
console.log('34 presets, 17 chassis, eight liveries, unchanged original slots/unlocks and versioned custom migration pass');
