...
let browser;
const extra = { Sign: 25, 'Building — village': 70, 'Building — snowy': 134, Barrel: 96, Cactus: 96, Cow: 208, Sheep: 144, Deer: 144, Goat: 232, Gull: 80, Pigeon: 88, Duck: 78, 'Sky train': 528, 'Tree — forest': 42 };
...
try {
  ...
  for (const name of names) {
    const row = await page.evaluate(async name => {
      ...
      v.freeze(0); v.orbit.theta = ['Cow', 'Sheep', 'Deer', 'Fox', 'Hare', 'Tortoise', 'Seal', 'Antelope', 'Boar', 'Frog'].includes(name) ? -0.8 : 0.7; v.orbit.phi = 1.1; v.orbit.radius *= name === 'Tower' ? 1.35 : name.startsWith('Tree') ? 1.28 : 1.06;
    ...
    const newBudgets = { Sailboat:[160,2], hut:[220,1], stiltHut:[316,1], cabin:[108,1], chalet:[108,1], adobe:[136,1], pavilion:[116,1], ruin:[172,1], Vulture:[386,5], Parrot:[362,3], 'Racing banner': [280, 2], 'Timber footbridge': [1700, 1], 'Tree — wetlands': [230, 2], 'Tree — lavender': [360, 2], 'Basalt columns': [72, 3] };
  ...
  // Chrome may exit before its close handshake resolves.
